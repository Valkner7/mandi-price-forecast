...
```
Retrains the global LightGBM model and backtests it against a naive "no change" baseline (P(t+1) = P(t)) on a held-out window (see `backtest_vs_naive()`). The results are written to `models/lgbm_price_model_meta.json` and also served live in `/predict`'s `confidence.validated_on` field, so the number in the product always matches what's actually in the trained artifact. As of the last retrain: 226 crop-mandi combinations tested, LightGBM beat the naive baseline in only ~4% of them — commodity prices behave close to a random walk over one day, documented honestly rather than hidden.
