"""Forecasting, dashboard, and advisory routes and logic.

Extracted out of app.py's monolith (Step 6 of the app.py breakup — see
HANDOFF_REPORT.md). This is the biggest single chunk: it owns the entire
forecasting pipeline (dataset loading/caching, the LightGBM/ETS model
loading and prediction path, anomaly detection), every route built on top
of it (/predict, /anomalies, /meta, /history, /trends, /trends-dashboard's
data source, /compare, /compare-advisory, /advisory, /voice-advisory), and
the nearby-mandis lookup (/api/nearby-mandis), which shares the same
cached dataframe even though it isn't a forecasting endpoint itself.

Like routers/voice.py and routers/alerts.py, this module imports a couple
of names (BASE_DIR, limiter) from `app` at load time. This only works
because app.py registers this router near the end of the file, after
those names are already defined there.

app.py, in turn, re-exports a few names FROM this module (_build_prediction,
generate_advisory, FORECAST_CONFIDENCE_NOTE) right after including this
router, so that routers/voice.py's and routers/alerts.py's existing
`from app import (...)` statements keep working unmodified — those two
routers were written to pull forecasting internals from `app`, and
changing that convention would mean touching already-committed files for
no functional benefit. See the router-registration block at the bottom of
app.py for the exact import order this depends on.

_warm_trends_cache() (used to pre-compute /trends on startup) is exported
WITHOUT an @router.on_event decorator, since APIRouter has no on_event of
its own — same situation as routers/alerts.py's
_maybe_start_internal_alert_scheduler. app.py registers it directly on the
app instance after including this router.
"""
...
from app import BASE_DIR, limiter
...
# APIRouter has no .on_event of its own (same situation as
# routers/alerts.py's _maybe_start_internal_alert_scheduler) — this is
# registered directly on the app instance in app.py's router-registration
# section, right after this router is included, instead of via a decorator
# here.
